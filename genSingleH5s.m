...
function getModData(inStruc, confgData)
    ...
    for modIndex = 1:numberOfMods
        ...
        exeName = [confgData.command.pythonStr ' fd_matchup.py', pyOpt,' > cmdOut.txt'];
        system(exeName);
        ...
        parfor (iii = 1:listLength),nworkers)
            ...
            %.m files and modify the xml accordingly.
                ...
                system(wgetString); %Beware that the system does not support UNC path names in windows, map the drives locally or use absolute paths.
            end           
